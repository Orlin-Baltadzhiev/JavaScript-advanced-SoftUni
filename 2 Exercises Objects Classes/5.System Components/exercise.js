function solve (input){
    ...
    input.forEach(line => {
       let[systemName, component, subCompoment] = line.split(" | ");
    ...
    result[systemName][component].push(subCompoment);
    })
    Object.entries(result).sort((curr,next) => {
        ...
        Object.entries(component).sort((curr,next) => {
            return next[1].length - curr[1].next;
        }).forEach(([component, subCompoment]) => {
            console.log(`|||${component}`)
            subCompoment.forEach( e => {
               console.log(`||||||${e}`);     
            });
        })
    })
